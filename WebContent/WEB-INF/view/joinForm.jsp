...
<div class="form-group">
    ...
    <input type="text" class="form-control" name="name" value="${param.name}" id="input2-name">
    <c:if test="${errors.name }"> 
    <small class="form-text text-danger">이름을 입력하세요.</small></c:if>
  </div>
  <div class="form-group">
    <label for="input3-pw">암호</label>
    <input type="password" class="form-control" id="input3-pw" name="pw">
    <c:if test="${errors.pw }">
     <small class="form-text text-danger">암호를 입력하세요.</small></c:if>
  </div>
  <div class="form-group">
    <label for="input4-comfirnPw">암호확인</label>
    <input type="password" class="form-control" id="input4-comfirnPw" name="confirmPw">
  ...
  <div class="form-group form-check">
    ...
    <label class="form-check-label" for="Check1">개인정보 활용에 동의 하십니까?</label>
  </div>
  		<div class="text-right">
  <button type="submit" class="btn btn-primary">회원 가입</button></div>
</form>
			</div>
